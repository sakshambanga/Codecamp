...
//   done(null, result);
// };
var constructPerson = function(done,data){
  var p1 = new Person({ name: data.name, age:data.age, favoriteFoods:data.favoriteFoods });
...
//constructPerson(done,{name:'Saksham Banga',age:19,favoriteFoods:['pizzas']});
...
var createAndSavePerson = function(done,data={name:'Saksham Banga',age:19,favoriteFoods:['pizzas']}) {
  var person = constructPerson(done,data);
...
var findPeopleByName = function(personName, done) {
  ...
  var result = Person.find({name:personName},done);
  return result;
  done(null/*, data*/);
...
var findAndUpdate = function(personName, done) {
  var ageToSet = 20;
  return Person.findOneAndUpdate({name:personName},{age:ageToSet},{new:true},done);
  //done(null/*, data*/);
};
...
var removeManyPeople = function(done) {
  ...
  return Person.remove({name:nameToRemove},function(err,data){if(err)return done(err);return done(null,data);})
  done(null/*, data*/);
};
